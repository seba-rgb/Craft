...
void interpolate_player(Player *player) {
  ...
  update_player(player, s1->x + (s2->x - s1->x) * p,
                ...
                s1->rx + (s2->rx - s1->rx) * p, s1->ry + (s2->ry - s1->ry) * p,
                0);
}
      ...
      if (control) {
        ...
        if (g->recording_macro == 0 && g->macro_count > 0) {
          ...
          for (int i = 0; i < g->macro_count; i++) {
            int y = g->macro_blocks[i].y - g->macro_blocks[0].y + hy;
            if (y > 0 && y < 256) {
              int x = g->macro_blocks[i].x - g->macro_blocks[0].x + hx;
              int z = g->macro_blocks[i].z - g->macro_blocks[0].z + hz;
...
int main(int argc, char **argv) {
  ...
  program =
      load_program("shaders/block_vertex.glsl", "shaders/block_fragment.glsl");
  ...
  while (running) {
    ...
    while (1) {
      ...
      if (SHOW_INFO_TEXT) {
        ...
        hour = hour % 12;
        hour = hour ? hour : 12;
        mlen = snprintf(text_buffer, 1024,
        ...
        if (g->recording_macro) {
          ...
          snprintf(text_buffer + mlen, remaining, " R%d", g->macro_count);
        }
