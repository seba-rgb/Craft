...
GLuint gen_cube_buffer(float x, float y, float z, float n, int w) {
    ...
    float ao[6][4] = {0};
    float light[6][4] = {{0.5, 0.5, 0.5, 0.5},
        {0.5, 0.5, 0.5, 0.5},
        {0.5, 0.5, 0.5, 0.5},
        {0.5, 0.5, 0.5, 0.5},
        {0.5, 0.5, 0.5, 0.5},
...
void draw_triangles_3d_ao(Attrib *attrib, GLuint buffer, int count) {
    ...
    glVertexAttribPointer(attrib->normal,
        3,
        GL_FLOAT,
        GL_FALSE,
        sizeof(GLfloat) * 10,
        (GLvoid *)(sizeof(GLfloat) * 3));
    glVertexAttribPointer(attrib->uv,
        4,
        GL_FLOAT,
        GL_FALSE,
        sizeof(GLfloat) * 10,
...
void sphere(Block *center, int radius, int fill, int fx, int fy, int fz) {
    static const float offsets[8][3] = {{-0.5, -0.5, -0.5},
        {-0.5, -0.5, 0.5},
        {-0.5, 0.5, -0.5},
        {-0.5, 0.5, 0.5},
        {0.5, -0.5, -0.5},
...
void parse_buffer(char *buffer) {
    ...
    while (line) {
        ...
        if (sscanf(
                line, "U,%d,%f,%f,%f,%f,%f", &pid, &ux, &uy, &uz, &urx, &ury) ==
            6) {
            me->id = pid;
            s->x = ux;
            s->y = uy;
        ...
        if (sscanf(
                line, "P,%d,%f,%f,%f,%f,%f", &pid, &px, &py, &pz, &prx, &pry) ==
            6) {
